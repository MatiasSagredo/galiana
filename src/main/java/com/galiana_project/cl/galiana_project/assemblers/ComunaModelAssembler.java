...
import com.galiana_project.cl.galiana_project.controller.ComunaControllerV2;
...
public class ComunaModelAssembler implements RepresentationModelAssembler<Comuna, EntityModel<Comuna>> {
    ...
    public EntityModel<Comuna> toModel(Comuna comuna) {
        return EntityModel.of(comuna,
                ...
                linkTo(methodOn(ComunaControllerV2.class).updateComuna(comuna.getId().longValue(), comuna)).withRel("actualizar"),
                linkTo(methodOn(ComunaControllerV2.class).deleteComuna(comuna.getId().longValue())).withRel("eliminar"),
                linkTo(methodOn(ComunaControllerV2.class).patchComuna(comuna.getId().longValue(), comuna)).withRel("actualizar-parcial"));
    }
}
