...
import com.galiana_project.cl.galiana_project.controller.AsientoControllerV2;
...
public class AsientoModelAssembler implements RepresentationModelAssembler<Asiento, EntityModel<Asiento>> {
    ...
    public EntityModel<Asiento> toModel(Asiento asiento) {
        return EntityModel.of(asiento,
                ...
                linkTo(methodOn(AsientoControllerV2.class).updateAsiento(asiento.getId().longValue(), asiento)).withRel("actualizar"),
                linkTo(methodOn(AsientoControllerV2.class).deleteAsiento(asiento.getId().longValue())).withRel("eliminar"),
                linkTo(methodOn(AsientoControllerV2.class).patchAsiento(asiento.getId().longValue(), asiento)).withRel("actualizar-parcial"));
    }
}
