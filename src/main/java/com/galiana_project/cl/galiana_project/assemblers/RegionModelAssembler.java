...
import com.galiana_project.cl.galiana_project.controller.RegionControllerV2;
...
public class RegionModelAssembler implements RepresentationModelAssembler<Region, EntityModel<Region>> {
    ...
    public EntityModel<Region> toModel(Region region) {
        return EntityModel.of(region,
                ...
                linkTo(methodOn(RegionControllerV2.class).updateRegion(region.getId().longValue(), region)).withRel("actualizar"),
                linkTo(methodOn(RegionControllerV2.class).deleteRegion(region.getId().longValue())).withRel("eliminar"),
                linkTo(methodOn(RegionControllerV2.class).patchRegion(region.getId().longValue(), region)).withRel("actualizar-parcial"));
    }
}
