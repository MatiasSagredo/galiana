...
import com.galiana_project.cl.galiana_project.controller.TeatroControllerV2;
...
public class TeatroModelAssembler implements RepresentationModelAssembler<Teatro, EntityModel<Teatro>> {
    ...
    public EntityModel<Teatro> toModel(Teatro teatro) {
        return EntityModel.of(teatro,
                ...
                linkTo(methodOn(TeatroControllerV2.class).updateTeatro(teatro.getId().longValue(), teatro)).withRel("actualizar"),
                linkTo(methodOn(TeatroControllerV2.class).deleteTeatro(teatro.getId().longValue())).withRel("eliminar"),
                linkTo(methodOn(TeatroControllerV2.class).patchTeatro(teatro.getId().longValue(), teatro)).withRel("actualizar-parcial"));
    }
}
