...
import com.galiana_project.cl.galiana_project.controller.SalaControllerV2;
...
public class SalaModelAssembler implements RepresentationModelAssembler<Sala, EntityModel<Sala>> {
    ...
    public EntityModel<Sala> toModel(Sala sala) {
        return EntityModel.of(sala,
                ...
                linkTo(methodOn(SalaControllerV2.class).updateSala(sala.getId().longValue(), sala)).withRel("actualizar"),
                linkTo(methodOn(SalaControllerV2.class).deleteSala(sala.getId().longValue())).withRel("eliminar"),
                linkTo(methodOn(SalaControllerV2.class).patchSala(sala.getId().longValue(), sala)).withRel("actualizar-parcial"));
