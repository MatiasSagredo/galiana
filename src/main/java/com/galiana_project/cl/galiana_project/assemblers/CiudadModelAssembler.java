...
import com.galiana_project.cl.galiana_project.controller.CiudadControllerV2;
...
public class CiudadModelAssembler implements RepresentationModelAssembler<Ciudad, EntityModel<Ciudad>>{
    ...
    public EntityModel<Ciudad> toModel(Ciudad ciudad) {
        return EntityModel.of(ciudad,
                ...
                linkTo(methodOn(CiudadControllerV2.class).updateCiudad(ciudad.getId().longValue(), ciudad)).withRel("actualizar"),
                linkTo(methodOn(CiudadControllerV2.class).deleteCiudad(ciudad.getId().longValue())).withRel("eliminar"),
                linkTo(methodOn(CiudadControllerV2.class).patchCiudad(ciudad.getId().longValue(), ciudad)).withRel("actualizar-parcial"));
    }
}
