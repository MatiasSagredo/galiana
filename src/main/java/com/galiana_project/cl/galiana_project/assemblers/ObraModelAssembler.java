...
import com.galiana_project.cl.galiana_project.controller.ObraControllerV2;
...
public class ObraModelAssembler implements RepresentationModelAssembler<Obra, EntityModel<Obra>> {
    ...
    public EntityModel<Obra> toModel(Obra obra) {
        return EntityModel.of(obra,
                ...
                linkTo(methodOn(ObraControllerV2.class).updateObra(obra.getId().longValue(), obra)).withRel("actualizar"),
                linkTo(methodOn(ObraControllerV2.class).deleteObra(obra.getId().longValue())).withRel("eliminar"),
                linkTo(methodOn(ObraControllerV2.class).patchObra(obra.getId().longValue(), obra)).withRel("actualizar-parcial"));
    }
}
